...
export function circularAverage(a: number, b: number): number {
...
export function selectBestAngle(
  ...
  const adjacent = matches.find(
    (candidate, index) =>
      index > 0 && circularDistance(best.matchedAngle, candidate.matchedAngle) === 45,
  );
  ...
  return { angle, confidence: best.confidence, matches };
}

export function turnToClockPosition(
...
export class LastMileService {
    ...
    try {
      ...
      if (!tiles) {
        res.status(422).json({
          ...
          uncertain: true,
          output:
            "I could not get a usable Google Street View panorama after two attempts. Please move a few meters, take another photo, and try Last Meters again.",
      ...
      if (!currentView) {
        res.status(200).json({
          ...
          retryable: true,
          output:
            "I found Street View imagery, but I am not confident enough to match your current photo to a direction. Please take another photo from where you are standing and try again.",
      ...
      if (!target) {
        ...
        if (!candidate) {
          res.status(200).json({
            ...
            uncertain: true,
            output: `I could not confidently identify ${destination} in the nearby Street View imagery or find a relevant nearby Google Maps result. Please confirm the destination name or take another photo.`,
        ...
        res.status(200).json({
          ...
          uncertain: true,
          output:
            `I could not confidently see the requested storefront in the nearby Street View imagery. ` +
            `The closest likely Google Maps result is ${candidate.name}, at ${candidate.address}, ` +
            `about ${candidate.estimatedBlocks} blocks (${candidate.walkingDistance}) away. ` +
            `Please confirm that this is the destination you want.`,
          candidate,
          confidence: Math.max(...targetComparisons.map((item) => item.confidence), 0),
          sources: [
            "Google Street View imagery",
      ...
      const landmarkUncertain = guidance.confidence < this.confidenceThreshold;
      const mode = turn.direction === "STRAIGHT" ? "aligned" : "exact";

      // The direction itself is derived from two confident panorama matches, so
      // always return degree + clock guidance even when landmark narration is weak.
      const guidanceText = landmarkUncertain
        ? `Direction match: ${turn.instruction} The storefront direction match is ${target.confidence}% confident. ` +
          `Visual landmark guidance is uncertain at ${guidance.confidence}% confidence: ${guidance.guidance} ` +
          `Please take another photo before relying on the landmark details.`
        : `Direction match: ${turn.instruction} The storefront direction match is ${target.confidence}% confident. ` +
          `${guidance.guidance} (${guidance.confidence}% visual-guidance confidence).`;

      res.status(200).json({
        code: "LAST_MILE_GUIDANCE",
        output: guidanceText,
        uncertain: landmarkUncertain,
        retryable: landmarkUncertain,
        mode,
        ...
        turn: {
          ...
          source: "Matched panorama angles from the user photo and target storefront",
        },
        guidance: {
  ...
  private async fetchValidatedPanoramaTiles(
    ...
    for (let attempt = 1; attempt <= PANORAMA_RETRY_COUNT; attempt += 1) {
      try {
        const metadataResponse = await axios.get<PanoramaMetadataResponse>(
  ...
  private async runComparison(
      ...
      messages: [
        {
          ...
          content:
            ...
            `{"comparisons":[{"heading":0,"match":true,"matchedAngle":0,"confidence":85,"position":"center"}]}. ` +
            "Include exactly one result for each heading 0,45,90,135,180,225,270,315.",
        },
    ...
    for (const raw of parsed?.comparisons ?? []) {
      ...
      byHeading.set(heading, {
        ...
        matchedAngle: Number.isFinite(Number(raw.matchedAngle))
          ? normalizeHeading(Number(raw.matchedAngle))
          : heading,
  ...
  private async findPlaceCandidate(
    ...
    const walkingDistance =
      leg?.distance?.text || (meters > 0 ? `${meters} m` : "distance unavailable");
    const estimatedBlocks =
      meters > 0 ? Math.max(1, Math.round(meters / APPROX_METERS_PER_BLOCK)) : 0;
  ...
  private async generateGuidance(
      ...
      messages: [
        {
          role: "system",
          content:
            `You are an orientation assistant for a Blind or Low Vision person. ` +
            `The destination is "${destination}". The system has calculated this direction: "${turnInstruction}". ` +
            "Use only the user's photo to describe useful physical or tactile landmarks that can help them orient safely. " +
            "Do not contradict the calculated direction. Do not rely on elevated text signs. " +
            'Return JSON only: {"guidance":"concise guidance","confidence":0}. ' +
            "Confidence must be an integer percentage from 0 to 100. If the image is unclear, lower confidence instead of inventing details.",
        },
        {
          role: "user",
    ...
    return {
      guidance:
        ...
        "I cannot identify a reliable physical landmark in this photo.",
